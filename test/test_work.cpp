...
TEST_CASE("Work batch cancellation")
{
  ...
  event_loop.run_in_loop(
    ...
    {
      task.emplace(
        [](vio::event_loop_t &el, vio::thread_pool_t &p, int ps) -> vio::task_t<void>
        {
          std::mutex mtx;
          ...
          while (threads_blocked.load(std::memory_order_acquire) < ps)
          {
          ...
          for (size_t i = ps; i < results.size(); ++i)
          {
            CHECK(!results[i].has_value());
    ...
    [&]
    {
      task.emplace(
        [](vio::event_loop_t &el, vio::thread_pool_t &p, int ps) -> vio::task_t<void>
        {
          ...
          std::condition_variable cv;
          std::atomic<int> threads_blocked{0};
          std::atomic<bool> release_item0{false};
          std::atomic<bool> release_others{false};
          ...
          for (int i = 0; i < ps; ++i)
          {
            work_items.emplace_back([&mtx, &cv, &threads_blocked, &release_item0, &release_others, i]() -> std::expected<int, vio::error_t>
            {
              threads_blocked.fetch_add(1, std::memory_order_release);
              if (i == 0)
              {
                // Item 0: wait for its own release, then fail
                std::unique_lock<std::mutex> lock(mtx);
                cv.wait(lock, [&release_item0] { return release_item0.load(std::memory_order_acquire); });
                return std::unexpected(vio::error_t{.code = -1, .msg = "forced failure"});
              }
              // Other items: wait for separate release so they stay blocked
              // while item 0 fails and sets the cancel flag
              std::unique_lock<std::mutex> lock(mtx);
              cv.wait(lock, [&release_others] { return release_others.load(std::memory_order_acquire); });
              return i;
            });
          }
          ...
          // Release only item 0 — it will fail and set the cancel flag
          {
            std::lock_guard<std::mutex> lock(mtx);
            release_item0.store(true, std::memory_order_release);
          }
          cv.notify_all();

          // Wait for the cancel flag to be set by item 0's failure
          while (!batch.state_ptr->cancelled.load(std::memory_order_acquire))
          {
            std::this_thread::yield();
          }

          // Now release the other blocked items — queued items will see the cancel flag
          {
            std::lock_guard<std::mutex> lock(mtx);
            release_others.store(true, std::memory_order_release);
          }
          cv.notify_all();
          ...
          // Queued items should all be cancelled (cancel flag set before threads released)
          for (size_t i = ps; i < results.size(); ++i)
          {
            CHECK(!results[i].has_value());
            CHECK(results[i].error().code == UV_ECANCELED);
    ...
    [&]
    {
      task.emplace(
        [](vio::event_loop_t &el, vio::thread_pool_t &p, int ps) -> vio::task_t<void>
        {
          ...
          for (int i = 0; i < ps; ++i)
          {
            work_items.emplace_back([&mtx, &cv, &threads_blocked, &release_item0, &release_others, i]() -> std::expected<int, custom_error_t>
            {
              threads_blocked.fetch_add(1, std::memory_order_release);
              if (i == 0)
              {
          ...
          // Now release the other blocked items
          {
            std::lock_guard<std::mutex> lock(mtx);
            release_others.store(true, std::memory_order_release);
          }
